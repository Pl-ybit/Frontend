    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-auto">
        {data.bids.map((r, i) => {
          const pct = calcBarPct(r.amount, maxAll)
          return (
            <div key={`bid-${i}`} className="relative grid grid-cols-3 px-4 py-[5px] text-xs hover:bg-emerald-500/5 cursor-pointer">
              <div
                className="absolute inset-y-0 right-0 bg-emerald-500/15"
                style={{ width: `${pct}%` }}
              />
              <span className="relative text-emerald-400 font-medium tabular-nums">{r.price.toLocaleString()}</span>
              <span className="relative text-center text-(--page-text) tabular-nums">{r.amount.toLocaleString()}</span>
              <span className="relative text-right text-(--text-muted) tabular-nums">{r.total.toLocaleString()}</span>
            </div>
          )
